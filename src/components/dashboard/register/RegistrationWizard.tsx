...
import { ShieldAlert, Shield, Loader2, AlertTriangle } from 'lucide-react';
  ...
  return (
    <>
      ...
      {step >= 0 && step < steps.length - 1 && (
        <div
          role="alert"
          className="mb-4 rounded-xl border border-amber-300/60 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-700/50 px-4 py-3 flex gap-3 items-start"
        >
          <AlertTriangle className="h-5 w-5 shrink-0 text-amber-600 dark:text-amber-400 mt-0.5" />
          <div className="space-y-1 min-w-0">
            <p className="text-sm font-semibold text-amber-900 dark:text-amber-200 leading-snug">
              ⚠️ {t('wizard.dontCloseTab.title')}
            </p>
            <p className="text-xs sm:text-[13px] text-amber-800/90 dark:text-amber-300/90 leading-relaxed">
              {t('wizard.dontCloseTab.body')}
            </p>
          </div>
        </div>
      )}
