  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-muted-foreground leading-snug mt-0.5">{sublabel}</p>
      </div>
      ...
      {isActive && <ChevronDown className="h-4 w-4 text-muted-foreground" />}
    </button>
  );
}
...
export function FirstHitFlow({ onSkip, onComplete }: { onSkip?: () => void; onComplete?: () => void }) {
      ...
      supabase
        ...
        .then(({ data }) => {
          if (data?.display_name) {
            ...
            setCreators((prev) => prev.map((c, i) => (i === 0 ? { ...c, name: data.display_name! } : c)));
          }
        });
    }
  }, [user]);
  ...
  const creatorsValid = creators.length >= 1 && creators.every((c) => c.name.trim() && c.roles.length > 0) && pctValid;
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!uploadFile && audioUrl) {
      ...
      if (!uploadFile) {
        setRegError(t("dashboard.firstHit.audioGenError"));
  ...
  const validateVisualFile = (file: File): Promise<string | null> => {
    ...
    if (![...VIDEO_EXTS, ...IMAGE_EXTS].includes(ext)) {
      return Promise.resolve(
        t("dashboard.premium.invalidVisualFormat", "Formato no válido. Sube un vídeo MP4/MOV o una imagen JPG/PNG."),
      );
    }
    ...
    if (IMAGE_EXTS.includes(ext)) {
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          resolve(
            isNineBySixteen(img.naturalWidth, img.naturalHeight)
              ? null
  ...
  const handlePromote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (spendData?.error === "insufficient_credits") {
        toast.error(
          t("dashboard.noCredits.costMessage", {
            action: t("dashboard.premium.submit"),
      ...
      <div className="flex items-center justify-center min-h-[70vh] px-4">
        <div className="max-w-lg w-full text-center space-y-8">
          ...
          <div className="space-y-3">
            <h2 className="text-2xl font-bold tracking-tight">{t("dashboard.firstHit.congratsTitle")}</h2>
            <p className="text-muted-foreground text-sm leading-relaxed">
              {t("dashboard.firstHit.congratsDesc")}
              <br />
              <br />
              En <strong>2–3 días</strong> tu canción aterrizará frente a <strong>+100.000 fans</strong> que están a
              punto de descubrirte. 😎
            </p>
          </div>
            ...
            <div className="px-5 pb-6 border-t border-border/40 pt-4 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  maxLength={2000}
                />
                <p className="text-[11px] text-muted-foreground text-right">{prompt.length}/2000</p>
              </div>
                      ...
                      >
                        ...
                        {v.sample_url && (
                          <span
                            ...
                            className={cn(
                              "inline-flex items-center gap-1 mt-1 text-[11px] cursor-pointer",
                              playingVoice === v.id ? "text-primary" : "text-muted-foreground",
                            )}
                          >
                            {playingVoice === v.id
                              ? t("dashboard.firstHit.voiceStop")
                              : t("dashboard.firstHit.voiceListen")}
                          </span>
                        )}
                      </button>
                    ))}
              ...
              {audioUrl && (
                <div className="rounded-xl border border-violet-500/30 bg-violet-500/5 p-4 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{t("dashboard.firstHit.aiGenerated", { n: "" })}</p>
                  </div>
                  <button
                    type="button"
                ...
                >
                  {generating ? (
                    <>
                      ...
                      {t("dashboard.firstHit.generating")}
                    </>
                  ) : (
                    <>
                      <Sparkles className="h-4 w-4" />
                      {t("dashboard.firstHit.generateAI")}
                    </>
                  )}
                </Button>
                <Button variant="outline" className="flex-1 gap-2" onClick={handleStep1Next}>
            ...
            <div className="px-5 pb-6 border-t border-border/40 pt-4">
              ...
              {regDone ? (
                ...
                <form onSubmit={handleRegister} className="space-y-3">
                    ...
                    {loadingSigs ? (
                      ...
                      <div className="space-y-2 p-3 rounded-xl border border-dashed border-amber-400/50 bg-amber-50/50 dark:bg-amber-900/10">
                        ...
                        {signatures.filter((s: IbsSignature) => s.status === "pending").length > 0 && (
                          <Button type="button" variant="ghost" size="sm" className="h-7 text-xs" onClick={loadSigs}>
                  ...
                  {/* Descripción */}
                  <div className="space-y-1.5">
                    <Label className="text-xs">{t("dashboard.firstHit.descriptionOptional")}</Label>
                    <Textarea
                      value={regDesc}
                  ...
                  <div className="space-y-3 pt-2">
                    <Label className="text-xs font-semibold">{t("dashboard.firstHit.creators")}</Label>
                        ...
                        <Card key={c.id} className="border-border/40">
                          <CardContent className="p-4 space-y-3">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              ...
                              <div className="space-y-1">
                                <Label className="text-xs">{t("dashboard.firstHit.emailOptional")}</Label>
                                <Input
                                  value={c.email}
                            ...
                            <div className="space-y-1">
                              <Label className="text-xs">{t("dashboard.firstHit.rightsPercent")}</Label>
                              <Input
                                type="number"
                                min={0}
                                max={100}
                    ...
                    {usesPercentages && totalPct !== 100 && (
                      <p className="text-xs text-destructive">{t("dashboard.firstHit.pctMustSum100")}</p>
                    )}
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    <Checkbox id="own" checked={ownership} onCheckedChange={(v) => setOwnership(!!v)} />
                    <Label htmlFor="own" className="text-xs leading-tight cursor-pointer">
                  ...
                  >
                    {registering ? (
                      <>
                        <Loader2 className="h-4 w-4 animate-spin" />
                        {t("dashboard.firstHit.registeringBlockchain")}
                      </>
                    ) : (
                      <>
                        <Shield className="h-4 w-4" />
                        {t("dashboard.firstHit.registerWork")}
                      </>
                    )}
                  </Button>
                  ...
                  {!skipWarning ? (
                    ...
                    <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-3 space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">{t("dashboard.firstHit.plagiarismRiskDesc")}</p>
                      <div className="flex gap-2">
                        <Button size="sm" className="flex-1 text-xs h-8" onClick={() => setSkipWarning(false)}>
          ...
          {activeStep === 3 && (
            <div className="px-5 pb-6 border-t border-border/40 pt-4 space-y-5">
              ...
              <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 space-y-2">
                ...
                <p className="text-[11px] text-muted-foreground/70 mt-1">{t("dashboard.premium.manualNote")}</p>
              </div>
              ...
              <form onSubmit={handlePromote} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {t(
                      "dashboard.premium.audioDesc",
                      "Sube tu propio audio para que nuestro equipo lo use en la promoción.",
                    )}
                  </p>
                  {promoAudioFile ? (
                    <div className="flex items-center gap-2 rounded-md border border-border/40 p-2 text-sm">
                      ...
                      <Button
                        ...
                        size="icon"
                        className="h-6 w-6"
                        onClick={() => setPromoAudioFile(null)}
                      >
                        <X className="h-3 w-3" />
                      </Button>
                    </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {t(
                      "dashboard.premium.visualDesc",
                      "Vídeo: MP4 o MOV, 1080×1920px (9:16). Imagen: JPG o PNG (9:16).",
                    )}
                  </p>
                  {promoMediaFile ? (
                    <div className="flex items-center gap-2 rounded-md border border-border/40 p-2 text-sm">
                      ...
                      <Button
                        ...
                        size="icon"
                        className="h-6 w-6"
                        onClick={() => setPromoMediaFile(null)}
                      >
                        <X className="h-3 w-3" />
                      </Button>
                    </div>
                ...
                {/* Consent */}
                <div className="flex items-start gap-2">
                  <Checkbox id="promo-consent" checked={promoConsent} onCheckedChange={(v) => setPromoConsent(!!v)} />
                  <Label htmlFor="promo-consent" className="text-xs leading-tight cursor-pointer">
                ...
                >
                  {promoting ? (
                    <>
                      <Loader2 className="h-4 w-4 animate-spin" />
                      {t("dashboard.firstHit.sendingRequest")}
                    </>
                  ) : (
                    <>
                      <Megaphone className="h-4 w-4" />
                      {t("dashboard.firstHit.wantPromotion")}
                    </>
                  )}
                </Button>
