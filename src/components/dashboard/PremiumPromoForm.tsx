          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-muted-foreground">
                {t('dashboard.premium.audioDesc', 'Sube tu propio audio para que nuestro equipo lo use en la promoción.')}
              </p>
              {audioFile ? (
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-muted-foreground">
                {t('dashboard.premium.visualDesc', 'Vídeo: MP4 o MOV, 1080×1920px (9:16). Imagen: JPG o PNG (9:16).')}
              </p>
              {mediaFile ? (
                <div className="flex items-center gap-2 rounded-md border border-border/40 p-2 text-sm">
                  ...
                  <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => setMediaFile(null)} aria-label={t('common.remove', { defaultValue: 'Eliminar' })}>
                    <X className="h-3 w-3" aria-hidden="true" />
                  </Button>
                </div>
              ) : (
            ...
            {noCredits ? (
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  disabled={submitting || !artistName.trim() || !songTitle.trim() || !lyrics.trim() || !audioFile || !mediaFile}
                  className="gap-2"
                >
                  {submitting ? (
          ...
          {loadingLyrics ? (
            ...
            <div className="space-y-2">
              {savedLyrics.map(item => (
                <button
                  key={item.id}
                  className="w-full text-left rounded-md border border-border/40 p-3 hover:border-primary/40 hover:bg-muted/30 transition-colors"
                  onClick={() => handleImportLyrics(item)}
                >
                  <p className="text-sm font-medium truncate">{item.theme || item.description || 'Sin título'}</p>
                  <p className="text-xs text-muted-foreground line-clamp-2 mt-1">{item.lyrics.slice(0, 120)}…</p>
