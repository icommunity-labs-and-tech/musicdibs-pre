...
const isPrimaryObjective = (objective) => PRIMARY_OBJECTIVES.has(objective);
const money = (value, currency) => new Intl.NumberFormat('es-ES', { style: 'currency', currency }).format(value);
export function GoogleAdsSpendPanel({ data, loading, error }) {
    if (loading) {
        return (<Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="rounded-lg border bg-emerald-50/50 dark:bg-emerald-950/20 p-4 space-y-3">
            ...
            <div className="max-h-44 overflow-y-auto">
              {primaryConversions.length === 0 ? <Empty /> : primaryConversions.map((row) => (<div key={row.objective} className="flex items-center justify-between gap-4 border-b last:border-0 border-emerald-200/60 dark:border-emerald-800/40 py-2 text-sm">
          ...
          <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
            ...
            <div className="max-h-44 overflow-y-auto">
              {secondaryConversions.length === 0 ? <Empty /> : secondaryConversions.map((row) => (<div key={row.objective} className="flex items-center justify-between gap-4 border-b last:border-0 py-2 text-sm">
        ...
        <div className="rounded-lg border p-4 space-y-3">
          ...
          <div className="max-h-56 overflow-y-auto">
            {data.campaign_spend.length === 0 ? <Empty /> : data.campaign_spend.map((row) => (<div key={row.campaign_name} className="flex items-center justify-between gap-4 border-b last:border-0 py-2 text-sm">
        ...
        {data.last_14_days && (<div className="rounded-md border bg-muted/10 p-4 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <Metric label="Conversiones principales" value={data.last_14_days.by_objective.filter((row) => isPrimaryObjective(row.objective)).reduce((sum, row) => sum + row.conversions, 0).toLocaleString('es-ES')} icon={<Target className="h-3.5 w-3.5 text-emerald-600"/>}/>
              <Metric label="Visitas / secundarias" value={data.last_14_days.by_objective.filter((row) => !isPrimaryObjective(row.objective)).reduce((sum, row) => sum + row.conversions, 0).toLocaleString('es-ES')} icon={<MousePointerClick className="h-3.5 w-3.5"/>}/>
            ...
            <div className="max-h-40 overflow-y-auto">
              {data.last_14_days.by_objective.length === 0 ? <Empty /> : data.last_14_days.by_objective.map((row) => (<div key={row.objective} className="flex items-center justify-between gap-4 border-b last:border-0 py-2 text-sm">
                  <span className={`min-w-0 truncate ${isPrimaryObjective(row.objective) ? 'font-medium' : 'text-muted-foreground'}`}>
                    {row.objective}
                    {!isPrimaryObjective(row.objective) && (<Badge variant="outline" className="ml-2 text-[10px] font-normal">visita</Badge>)}
                  </span>
...
function Metric({ label, value, icon }) {
    return (<div className="rounded-md border bg-muted/20 p-3 space-y-1">
