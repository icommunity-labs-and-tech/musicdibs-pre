...
export const GenerationWarning = () => {
  ...
  return (
    <div
      role="alert"
      className="w-full rounded-xl border border-warning/60 bg-warning dark:bg-warning/30 dark:border-warning/50 px-4 py-3 flex gap-3 items-start"
    >
      <AlertTriangle className="h-5 w-5 shrink-0 text-warning dark:text-warning mt-0.5" />
      <div className="space-y-1 min-w-0">
        <p className="text-sm font-semibold text-warning dark:text-warning leading-snug">
          {t('aiShared.dontCloseTab.title', { defaultValue: '⚠️ NO cierres esta pestaña durante la generación' })}
        </p>
        <p className="text-xs sm:text-[13px] text-warning/90 dark:text-warning/90 leading-relaxed">
