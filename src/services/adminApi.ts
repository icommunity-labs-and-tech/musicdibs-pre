...
export const adminApi = {
  getUsers: (offset = 0, search = '', limit = 50, filters: { kyc_filter?: string; plan_filter?: string; stripe_filter?: string; status_filter?: string; role_filter?: string; credits_filter?: string; reminder_filter?: string; sort_by?: string; sort_dir?: 'asc' | 'desc' } = {}) =>
  ...
  getWorkAudioUrl: (work_id: string, expires_in_seconds?: number) => adminAction('get_work_audio_url', { work_id, expires_in_seconds }) as Promise<{ signed_url: string; original_filename: string | null }>,
