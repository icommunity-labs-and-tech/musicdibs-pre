...
serve(async (req) => {
  ...
  try {
    ...
    if (!isFree) {
      ...
      const REGEN_CREDIT_COST = await getOperationCost(supabase, 'promo_social_regenerate_image', 5);
      const { data: deductResult, error: deductError } = await supabase.rpc('deduct_credits_ordered', {
        p_user_id: user.id, p_amount: REGEN_CREDIT_COST, p_feature: 'promo_social_regenerate_image',
        p_description: `Regeneración imagen (pagada)`,
      });
